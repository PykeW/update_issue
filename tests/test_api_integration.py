...
class TestManualSync(unittest.TestCase):
    ...
    def test_manual_sync_import(self):
        """测试手动同步模块导入"""
        try:
            from src.gitlab.services import manual_sync
            self.assertTrue(hasattr(manual_sync, 'main'))
...
class TestHealthCheck(unittest.TestCase):
    ...
    def test_health_check_import(self):
        """测试健康检查模块导入"""
        try:
            from src.gitlab.services import health_check
            self.assertTrue(hasattr(health_check, 'main'))
        except ImportError as e:
